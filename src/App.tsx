...
import type { Project, Story, Task, User, StoryPriority, StoryStatus, TaskPriority, TaskStatus } from './types';
...
export default function App() {
  ...
  const handleProjectSubmit = (e: React.FormEvent) => {
    ...
    if (editingProject) { ProjectService.update({ ...editingProject, ...projectForm }); setEditingProject(null); }
    else ProjectService.add(projectForm);
  ...
  const handleTaskSubmit = async (e: React.FormEvent) => {
    ...
    if (editingTask) { await TaskService.update({ ...editingTask, ...taskForm }); setEditingTask(null); }
    else await TaskService.create({ ...taskForm, historijaId: activeStoryId, stan: 'todo' });
  ...
  const handleAssignUser = async () => {
    if (!activeTaskId || !assignUserId) return;
      ...
      <div className="mx-auto max-w-6xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1 text-xs text-slate-400">
                ...
                {activeStory && (view === 'tasks' || view === 'task-detail') && (<>
                  <ChevronRight className="h-3 w-3" />
                  <span className={`cursor-pointer hover:text-indigo-600 ${view === 'tasks' ? 'text-indigo-600 font-semibold' : ''}`}
                    onClick={() => setView('tasks')}>{activeStory.nazwa}</span>
                </>)}
                {activeTask && view === 'task-detail' && (<>
                  <ChevronRight className="h-3 w-3" />
                  <span className="text-indigo-600 font-semibold">{activeTask.nazwa}</span>
                </>)}
              </div>
            </div>
          </div>
                    ...
                    {filteredStories.filter(s => s.stan === status).map(story => (
                      <Card key={story.id} className="bg-white dark:bg-slate-800 shadow-sm hover:border-indigo-200 transition-colors">
                        ...
                        <CardFooter className="flex gap-2 border-t pt-3">
                          ...
                          <Button variant="ghost" size="sm" className="h-8 text-red-500 hover:bg-red-50" onClick={() => handleDeleteStory(story.id)}><Trash2 className="h-3 w-3" /></Button>
                        </CardFooter>
                      </Card>
                    ))}
                  </div>
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-slate-50 rounded-lg p-3 col-span-2">
                    <p className="text-xs text-slate-400 mb-1">Przypisana osoba</p>
                    <p className="font-medium text-xs">{getUserName(activeTask.uzytkownikId)}</p>
                  </div>
                </div>
                ...
                {activeTask.stan === 'todo' && (
                  <div className="border rounded-lg p-4 space-y-3 bg-blue-50/50">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button onClick={handleAssignUser} disabled={!assignUserId} className="bg-blue-600 hover:bg-blue-700">Przypisz</Button>
                    </div>
                    <p className="text-xs text-slate-400">Przypisanie automatycznie zmieni stan na DOING i uzupełni datę startu.</p>
                  </div>
                )}

                {activeTask.stan === 'doing' && (
                  <div className="border rounded-lg p-4 space-y-3 bg-green-50/50">
                    <p className="text-sm font-semibold flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-green-500" /> Zamknij zadanie</p>
                    {!showDoneModal ? (
                      <Button onClick={() => setShowDoneModal(true)} className="bg-green-600 hover:bg-green-700">Oznacz jako DONE</Button>
                    ) : (
                      <div className="flex gap-2 items-center flex-wrap">
                        <Input type="number" min={0} placeholder="Roboczogodziny" value={doneHours || ''} onChange={e => setDoneHours(Number(e.target.value))} className="max-w-[150px]" />
                        <span className="text-sm text-slate-500">godz.</span>
                        <Button onClick={handleMarkDone} className="bg-green-600 hover:bg-green-700">Potwierdź</Button>
                        <Button variant="ghost" onClick={() => setShowDoneModal(false)}>Anuluj</Button>
                      </div>
                    )}
                    <p className="text-xs text-slate-400">Zamknięcie uzupełni datę zakończenia. Jeśli wszystkie zadania historyjki będą gotowe, historyjka też zmieni stan na DONE.</p>
                  </div>
                )}

                {activeTask.stan === 'done' && (
                  <div className="rounded-lg p-4 bg-green-50 flex items-center gap-3 text-green-700">
                    <CheckCircle2 className="h-5 w-5" />
                    <span className="text-sm font-semibold">Zadanie zakończone</span>
                  </div>
                )}
              </CardContent>
              <CardFooter className="gap-2 border-t">
                <Button variant="outline" size="sm" onClick={() => { setEditingTask(activeTask); setTaskForm({ nazwa: activeTask.nazwa, opis: activeTask.opis, priorytet: activeTask.priorytet, przewidywanyCzas: activeTask.przewidywanyCzas }); setView('tasks'); }}>
                  <Pencil className="h-3 w-3 mr-2" /> Edytuj
                </Button>
                <Button variant="ghost" size="sm" className="text-red-500 hover:bg-red-50" onClick={() => { handleDeleteTask(activeTask.id); setView('tasks'); }}>
        ...
        {view === 'kanban' && (
          <div className="space-y-4">
            ...
            {loading && <p className="text-slate-400 text-sm">Ładowanie...</p>}
                      ...
                      return (
                        <Card key={task.id} className="bg-white dark:bg-slate-800 shadow-sm hover:border-indigo-200 transition-colors cursor-pointer text-sm"
                          ...
                          <CardContent className="px-3 pb-3">
                            <p className="text-xs text-slate-500 line-clamp-2">{task.opis}</p>
                            {task.uzytkownikId && <p className="text-[10px] text-slate-400 mt-1"><UserIcon className="h-3 w-3 inline mr-1" />{getUserName(task.uzytkownikId)}</p>}
                          </CardContent>
                        </Card>
                      );
                    })}
                    {colTasks.length === 0 && <p className="text-xs text-slate-300 text-center py-4">Brak zadań</p>}
                  </div>
                );
              })}
            </div>
